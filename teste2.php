...
function splitVideo($videoPath, $outputDirectory, $ffmpegPath)
{
    ...
    // Define o formato de vídeo e áudio para os segmentos
    $videoFormat = "-c:v libx264 -preset medium -crf 22";
    $audioFormat = "-c:a aac -b:a 128k";

    // Define o comando FFmpeg para dividir o vídeo
    $command = "$ffmpegPath -i \"$videoPath\" -map 0 -f segment -segment_time $segmentDuration $videoFormat $audioFormat \"$outputDirectory/part-%03d.mp4\"";
